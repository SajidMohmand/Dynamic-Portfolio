...
const AdminCourse = () => {
    ...
    const onFinish = async (values) => {
        try {
            ...
            if (selectedItemForEdit) {
                response = await axios.post("http://localhost:5000/api/portfolio/update-course",
                    {
                        ...values,
    ...
    const onDelete = async (item) => {
        ...
        try {
            dispatch(ShowLoading());
            const response = await axios.post("http://localhost:5000/api/portfolio/delete-course", {
                _id: item._id,
            })
            dispatch(HideLoading());
            if (response.data.success) {
